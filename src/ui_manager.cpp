...
void UIManager::init(SystemData* sysData, GPSData* gpsData, IMUData* imuData, 
    ...
    // CRITICAL: Null pointer checks to prevent crashes
    if (!sysData || !gpsData || !imuData || !battData || !perfData) {
        Serial.println("❌ UIManager: Null data pointers provided!");
        return;
    }
    ...
    if (!initializeHardware()) {
        Serial.println("❌ Failed to initialize display hardware");
        return;
    }
...
bool UIManager::initializeHardware() {
    ...
    try {
        ...
        if (!bus) {
            Serial.println("❌ Failed to create display bus");
            return false;
        }
...
bool UIManager::initializeTouch() {
    ...
    try {
        ...
        Serial.println("❌ Exception during touch initialization");
        return false;
    }
}
...
void UIManager::initializeLVGL() {
    ...
    indev_drv.user_data = this;  // CRITICAL: Set user data to this instance
...
void UIManager::lvgl_touch_read(lv_indev_drv_t *indev_driver, lv_indev_data_t *data) {
    // CRITICAL: Always initialize data to safe state first
    data->state = LV_INDEV_STATE_REL;
    data->point.x = 0;
    data->point.y = 0;
    
    // Get instance from user data with null check
    UIManager* instance = (UIManager*)indev_driver->user_data;
    if (!instance) {
        return;  // No instance, return safely
    }
...
void UIManager::createMainLayout() {
    try {
        ...
        Serial.println("❌ Exception during UI layout creation");
    }
}
...
void UIManager::createHeader() {
    ...
    if (titleLabel) {
        ...
        lv_obj_set_style_text_font(titleLabel, UI_FONT_SMALL, 0);
...
void UIManager::createSpeedometerScreen() {
    ...
    if (speedValue) {
        ...
        lv_obj_set_pos(speedValue, 80, 50);
    }
...
void UIManager::createMotionScreen() {
    ...
    // Create accelerometer labels with null checks
...
void UIManager::createSystemScreen() {
    ...
    // Create system screen labels with null checks
...
void UIManager::createPerformanceScreen() {
    ...
    // Create performance screen labels with null checks
...
void UIManager::updateFileTransferUI() {
    ...
    } else if (progressBar && transferLabel) {
...
void UIManager::updateHeader() {
    // CRITICAL: Always check for null pointers before accessing data
    if (gpsData && timeLabel) {
    ...
    // Update battery display with null checks
    if (batteryData && batteryLabel && batteryIcon) {
        char battStr[16];
...
void UIManager::updateStatusBar() {
    ...
    // BLE status (placeholder) with null check
    if (bleStatus) {
        lv_obj_set_style_text_color(bleStatus, UI_COLOR_PRIMARY, 0);
    }
    
    // SD status with null check
    if (sdStatus) {
...
void UIManager::updateSpeedometerScreen() {
    // CRITICAL: Always check for null pointers before accessing GPS data
    if (!gpsData || !speedValue) return;
    
    // Update speed display with bounds checking
    if (gpsData->fixType >= 2 && gpsData->speed >= 0 && gpsData->speed < 1000) {
        char speedStr[16];
        if (gpsData->speed < 10) {
        ...
        // Unit label with null check
        if (speedUnit) {
            lv_label_set_text(speedUnit, "km/h");
        }
        
        // Coordinates with bounds checking
        if (coordsLabel && gpsData->latitude >= -90 && gpsData->latitude <= 90 && 
        ...
        // Altitude and heading with bounds checking
        if (altitudeLabel && gpsData->altitude >= -1000 && gpsData->altitude <= 10000) {
            char altStr[32];
            snprintf(altStr, sizeof(altStr), "ALT: %dm", gpsData->altitude);
            lv_label_set_text(altitudeLabel, altStr);
        }
        
        if (headingLabel && gpsData->heading >= 0 && gpsData->heading <= 360) {
            char hdgStr[32];
            snprintf(hdgStr, sizeof(hdgStr), "HDG: %.0f°", gpsData->heading);
            lv_label_set_text(headingLabel, hdgStr);
        }
        
        // Date with bounds checking
        if (dateLabel && gpsData->year >= 2000 && gpsData->year <= 2100 &&
        ...
        // No fix or invalid data
        lv_label_set_text(speedValue, "---");
        lv_obj_set_style_text_color(speedValue, UI_COLOR_DANGER, 0);
        
        if (speedUnit) lv_label_set_text(speedUnit, "");
        if (speedMph) lv_label_set_text(speedMph, "-- mph");
        if (coordsLabel) lv_label_set_text(coordsLabel, "LAT: -- LON: --");
        if (altitudeLabel) lv_label_set_text(altitudeLabel, "ALT: --m");
        if (headingLabel) lv_label_set_text(headingLabel, "HDG: --°");
        if (dateLabel) lv_label_set_text(dateLabel, "--/--/--");
    }
    
    // Update acceleration indicator with null checks
    static float lastSpeed = 0;
    if (imuData && systemData && systemData->mpuAvailable && accelerationIndicator && gpsData) {
...
void UIManager::updateMotionScreen() {
    // CRITICAL: Always check for null pointers before accessing IMU data
    if (!imuData) return;
    
    // Update motion indicator with null check
    if (motionIndicator) {
    ...
    // Update gyroscope data with bounds checking
    if (gyroXLabel && imuData->gyroX >= -2000 && imuData->gyroX <= 2000) {
        char gyroStr[32];
        snprintf(gyroStr, sizeof(gyroStr), "X: %.1f", imuData->gyroX);
        lv_label_set_text(gyroXLabel, gyroStr);
    }
    
    if (gyroYLabel && imuData->gyroY >= -2000 && imuData->gyroY <= 2000) {
        char gyroStr[32];
        snprintf(gyroStr, sizeof(gyroStr), "Y: %.1f", imuData->gyroY);
        lv_label_set_text(gyroYLabel, gyroStr);
    }
    
    if (gyroZLabel && imuData->gyroZ >= -2000 && imuData->gyroZ <= 2000) {
        char gyroStr[32];
    ...
    if (temperatureLabel && imuData->temperature >= -50 && imuData->temperature <= 150) {
        char tempStr[32];
...
void UIManager::updateSystemScreen() {
    // CRITICAL: Always check for null pointers before accessing system data
    if (!systemData) return;
    
    // Update connectivity status with null checks
    char statusStr[64];
    
    // WiFi with null check
    if (wifiInfoLabel) {
        if (WiFi.status() == WL_CONNECTED) {
    ...
    // Battery info with null checks
    if (batteryData && batteryInfoLabel) {
    ...
    if (batteryData && chargingInfoLabel) {
    ...
    // Logging status with null check
    if (logStatusLabel) {
...
void UIManager::updatePerformanceScreen() {
    // CRITICAL: Always check for null pointers before accessing performance data
    if (!perfStats) return;
    
    char perfStr[64];
    
    // Update throughput data with null checks
    if (totalPacketsLabel) {
    ...
    // Calculate and display drop rate with null check
    if (dropRateLabel) {
    ...
    // Update timing data with null checks
    if (minDeltaLabel) {
    ...
    // Calculate and display data rate with null checks
    if (dataRateLabel && perfStatusLabel) {
    ...
    // Update memory info with null check
    if (memoryLabel) {
        uint32_t freeHeap = ESP.getFreeHeap();
...
void UIManager::showScreen(ScreenType screen) {
    ...
    if (titleLabel && screen >= 0 && screen < 4) {
...
void UIManager::hideAllScreens() {
    if (speedometerPanel) lv_obj_add_flag(speedometerPanel, LV_OBJ_FLAG_HIDDEN);
    if (motionPanel) lv_obj_add_flag(motionPanel, LV_OBJ_FLAG_HIDDEN);
    if (systemPanel) lv_obj_add_flag(systemPanel, LV_OBJ_FLAG_HIDDEN);
    if (performancePanel) lv_obj_add_flag(performancePanel, LV_OBJ_FLAG_HIDDEN);
}

void UIManager::showCurrentScreen() {
    switch (currentScreen) {
        case SCREEN_SPEEDOMETER:
            if (speedometerPanel) lv_obj_clear_flag(speedometerPanel, LV_OBJ_FLAG_HIDDEN);
            break;
        case SCREEN_MOTION:
            if (motionPanel) lv_obj_clear_flag(motionPanel, LV_OBJ_FLAG_HIDDEN);
            break;
        case SCREEN_SYSTEM:
            if (systemPanel) lv_obj_clear_flag(systemPanel, LV_OBJ_FLAG_HIDDEN);
            break;
        case SCREEN_PERFORMANCE:
            if (performancePanel) lv_obj_clear_flag(performancePanel, LV_OBJ_FLAG_HIDDEN);
            break;
    }
}
...
void UIManager::buttonEventHandler(lv_event_t* e) {
    ...
    if (target == ui->prevButton) {
        ...
        // Update button appearance with null checks
        if (ui->systemData && ui->logButton) {
            if (ui->systemData->loggingActive) {
...
void UIManager::screenEventHandler(lv_event_t* e) {
    ...
    if (ui->currentScreen == SCREEN_PERFORMANCE) {
        // Reset performance stats on center touch with null check
        if (ui->perfStats) {
